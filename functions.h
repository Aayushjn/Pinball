...
bool atMax = false;
int score = 0;
vector<float> line_ptsx;
vector<float> line_ptsy;
...
void lineDDA(float x0, float y0, float xEnd, float yEnd){
    ...
    for(int k = 0; k < steps; k++){
        x += xIncrement;
        y += yIncrement;
        //if(((int)x==(int)X[2] && (int)y==(int)Y[2]) || ((int)x==(int)X[3] && (int)y==(int)Y[3]))
        //detect_collision(round(x), round(y));
...
void detect_collision(){
    ...
    if(ballY-30==ballX*slope1+c1 || ballY-30==ballX*slope2+c2 ||ballY-30==ballX*slope1+c1+10 ||ballY-30==ballX*slope1+c1-10||ballY-30==ballX*slope2+c2+10||ballY-30==ballX*slope2+c2-10){
        ySpeed=-ySpeed;
        xSpeed=-xSpeed;
    }
    //bounceBall();
